...
int IppContourTracing(U16* fpga_videodata, int sx, int sy, Edge* edgeCord)
{
	...
	while (1)
	{
		...
		if (nx < 0 || nx >= WIDTH || ny < 0 || ny >= HEIGHT || fpga_videodata[nx*WIDTH+ny] == 0xffff)
		{
			...
			if (cnt >= 8)
			{
				edgeCord[Num_edge_pixel].x = nx;
				edgeCord[Num_edge_pixel].y = ny;
				Num_edge_pixel++;
				fpga_videodata[nx*WIDTH + ny] = 0xf800;
				break;  // 외곽선 추적을 끝냄.
			}
		}
		else
...
static void demo(void)
{
	...
	while (i--)
	{
		...
		for (x_index=5;x_index < WIDTH-5; x_index++)
		{
			for(y_index=5;y_index<HEIGHT-5;y_index++)
			{
				Num_edge_pixel = IppContourTracing(fpga_videodata,x_index,y_index, edgeCord);
				if (Num_edge_pixel > 70)
					break;
			}
			if(Num_edge_pixel > 70) 
				break;
		...
		flip();
		printf("%d\n", i);
		for(i=0; i<Num_edge_pixel; i++) {
			printf("(%d, %d)\n", edgeCord[i].x, edgeCord[i].y);
		}
		if(Num_edge_pixel > 70) 
			break;
	}
